...
var game = new Phaser.Game(gameWidth, gameHeight, Phaser.CANVAS, "gameCanvas");
...
var main = function () {
  ...
  function createLeftNote(speed) {
    ...
    const timer = setTimeout(() => {
      if (noteArrL[noteAddL - 1]) noteArrL[noteAddL - 1].alpha = 1;
      clearTimeout(timer);
    }, 100);
    noteAddL++;
  }
  ...
  function createCenterNote(speed) {
    ...
    const timer = setTimeout(() => {
      if (noteArrC[noteAddC - 1]) noteArrC[noteAddC - 1].alpha = 1;
      clearTimeout(timer);
    }, 100);
    noteAddC++;
  }
  ...
  function createRightNote(speed) {
    ...
    const timer = setTimeout(() => {
      if (noteArrR[noteAddR - 1]) noteArrR[noteAddR - 1].alpha = 1;
      clearTimeout(timer);
    }, 100);
    noteAddR++;
  }
    ...
    game.time.events.add(
      ...
      function () {
        ...
        if (currentLevel <= levelNum) {
          ...
          // 播放升级动画
          game.add.tween(boxGroup).to({ y: boxGroup.y - 100, alpha: 0 }, 200, "Linear", true);
        }
      },
      this
    );
